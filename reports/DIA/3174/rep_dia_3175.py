...
report_name = '3175 - Количество получателей и сумма в разрезе количества детей'
report_code = '3175'
...
SELECT
   ...
   COUNT(UNIQUE CASE WHEN rfpm in ('07050001', '07050101') THEN PNCD_ID END) CNT01,
   SUM(CASE WHEN rfpm in ('07050001', '07050101') THEN SUM_PAY ELSE 0 END) SUM01,
   round(AVG(CASE WHEN rfpm in ('07050001','07050101') THEN ps ELSE NULL END), 2) AVG01,
   COUNT(UNIQUE CASE WHEN rfpm in ('07050002','07050102') THEN PNCD_ID END) CNT02,
   SUM(CASE WHEN rfpm in ('07050002', '07050102') THEN SUM_PAY ELSE 0 END) SUM02,
   round(AVG(CASE WHEN rfpm in ('07050002', '07050102') THEN ps ELSE NULL END), 2) AVG02,
   COUNT(UNIQUE CASE WHEN rfpm in ('07050003', '07050103') THEN PNCD_ID END) CNT03,
   SUM(CASE WHEN rfpm in ('07050003', '07050103') THEN SUM_PAY ELSE 0 END) SUM03,
   round(AVG(CASE WHEN rfpm in ('07050003', '07050103') THEN ps ELSE NULL END), 2) AVG03,
   COUNT(UNIQUE CASE WHEN rfpm in ('07050004', '07050104') THEN PNCD_ID END) CNT04,
   SUM(CASE WHEN rfpm in ('07050004', '07050104') THEN SUM_PAY ELSE 0 END) SUM04,
   round(AVG(CASE WHEN rfpm in ('07050004', '07050104') THEN ps ELSE NULL END), 2) AVG04,
...
def format_worksheet(worksheet, common_format):
    ...
    worksheet.merge_range(2, 2, 2, 13, 'По уходу за', common_format)

    worksheet.merge_range(3, 2, 3, 4, 'Первым ребенком', common_format)
    worksheet.merge_range(3, 5, 3, 7, 'Вторым ребенком', common_format)
    worksheet.merge_range(3, 8, 3, 10, 'Третьим ребенком', common_format)
    worksheet.merge_range(3, 11, 3, 13, 'Четвертым и более ребенком', common_format)
