...
report_name = '3186 - Количество работодателей'
report_code = '3186'

stmt_report = """
SELECT
  c,
  COUNT(DISTINCT sipr_ID) CNTP,
  COUNT(DISTINCT sicp_ID) CNT,
  round(AVG(SUM_ALL), 2) AVGS
FROM (
SELECT
  sfa.sipr_id,
  sfa.sicp_id,
  sfa.sum_all,
  COUNT(DISTINCT pd.p_rnn) c
FROM SIPR_MAKET_FIRST_APPROVE_2 sfa, em5_sird_reckon_donation sird, pmpd_pay_doc pd
 WHERE RFPM_ID LIKE '0705%'
   ...
   AND sird.mhmh_id = pd.mhmh_id
GROUP BY sfa.sipr_id, sfa.sicp_id, sfa.sum_all)
GROUP BY c
ORDER BY c
"""
...
def format_worksheet(worksheet, common_format):
    ...
    worksheet.write(2, 0, 'Количество работодателей', common_format)
    worksheet.write(2, 1, 'Количество назначений', common_format)
    worksheet.write(2, 2, 'Количество, человек', common_format)
    worksheet.write(2, 3, 'Средний назначенный размер', common_format)
