...
report_name = '3180 - Сведения о назначенных средних размерах по областям'
report_code = '3180'
...
SELECT
    br.RFBN_ID,
    br.NAME,
    round(AVG(CASE WHEN rfpm_id in ('07050001', '07050101') THEN sfa.sum_all ELSE NULL END), 2) AV01,
    round(AVG(CASE WHEN rfpm_id in ('07050002', '07050102') THEN sfa.sum_all ELSE NULL END), 2) AV02,
    round(AVG(CASE WHEN rfpm_id in ('07050003', '07050103') THEN sfa.sum_all ELSE NULL END), 2) AV03,
    round(AVG(CASE WHEN rfpm_id in ('07050004', '07050104') THEN sfa.sum_all ELSE NULL END), 2) AV04,
    round(AVG(sfa.sum_all), 2) AV
  FROM SIPR_MAKET_FIRST_APPROVE_2 sfa, rfbn_branch br
   WHERE
     SUBSTR(sfa.RFBN_ID, 1, 2) || '00' = br.RFBN_ID
     AND RFPM_ID LIKE '0705%'
...
def format_worksheet(worksheet, common_format):
    ...
    worksheet.write(3, 1, 'Первым ребенком', common_format)
    worksheet.write(3, 2, 'Вторым ребенком', common_format)
    worksheet.write(3, 3, 'Третьим ребенком', common_format)
    worksheet.write(3, 4, 'Четвертым и более ребенком', common_format)
