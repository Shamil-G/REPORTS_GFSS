...
report_name = '3182 - Количество получателей и сумма выплаты по регионам'
report_code = '3182'
...
SELECT
  ...
  ROUND(SUM(CASE WHEN rfpm in ('07050001', '07050101') THEN sm END)/COUNT(DISTINCT CASE WHEN rfpm in ('07050001', '07050101') THEN PNCD_ID END)/Months_Between(trunc(TO_DATE(:dt_to,'YYYY-MM-DD'), 'MM'), trunc(TO_DATE(:dt_from,'YYYY-MM-DD'), 'MM')) + 1) avg01,
  ...
  ROUND(SUM(CASE WHEN rfpm in ('07050002', '07050102') THEN sm END)/COUNT(DISTINCT CASE WHEN rfpm in ('07050002', '07050102') THEN PNCD_ID END)/Months_Between(trunc(TO_DATE(:dt_to,'YYYY-MM-DD'), 'MM'), trunc(TO_DATE(:dt_from,'YYYY-MM-DD'), 'MM')) + 1) avg02,
  ...
  ROUND(SUM(CASE WHEN rfpm in ('07050003', '07050103') THEN sm END)/COUNT(DISTINCT CASE WHEN rfpm in ('07050003', '07050103') THEN PNCD_ID END)/Months_Between(trunc(TO_DATE(:dt_to,'YYYY-MM-DD'), 'MM'), trunc(TO_DATE(:dt_from,'YYYY-MM-DD'), 'MM')) + 1) avg03,
  ...
  ROUND(SUM(CASE WHEN rfpm in ('07050004', '07050104') THEN sm END)/COUNT(DISTINCT CASE WHEN rfpm in ('07050004', '07050104') THEN PNCD_ID END)/Months_Between(trunc(TO_DATE(:dt_to,'YYYY-MM-DD'), 'MM'), trunc(TO_DATE(:dt_from,'YYYY-MM-DD'), 'MM')) + 1) avg04,
...
def format_worksheet(worksheet, common_format):
    ...
    worksheet.merge_range(2, 1, 2, 12, 'По уходу за', common_format)

    worksheet.merge_range(3, 1, 3, 3, 'Первым ребенком', common_format)
    worksheet.merge_range(3, 4, 3, 6, 'Вторым ребенком', common_format)
    worksheet.merge_range(3, 7, 3, 9, 'Третьим ребенком', common_format)
    worksheet.merge_range(3, 10, 3, 12, 'Четвертым и более ребенком', common_format)
    ...
    with oracledb.connect(user=db_user, password=db_password, dsn=db_dsn) as connection:
        with connection.cursor() as cursor:
            ...
            for col in range(1, 16):
                ...
                fmt = total_money_format if col in (2, 5, 8, 11, 14) else total_digital_format
