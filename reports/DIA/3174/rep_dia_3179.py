...
report_name = '3179 - Количество месяцев принятых в расчет при назначении'
report_code = '3179'

stmt_report = """
SELECT
  mc,
  COUNT(DISTINCT CASE WHEN rfpm_id in ('07050001', '07050101') THEN sipr_id END) CNT01,
  COUNT(DISTINCT CASE WHEN rfpm_id in ('07050002', '07050102') THEN sipr_id END) CNT02,
  COUNT(DISTINCT CASE WHEN rfpm_id in ('07050003', '07050103') THEN sipr_id END) CNT03,
  COUNT(DISTINCT CASE WHEN rfpm_id in ('07050004', '07050104') THEN sipr_id END) CNT04,
  COUNT(sipr_id) CNT
FROM (
SELECT
  sfa.sipr_id,
