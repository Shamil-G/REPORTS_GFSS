...
report_name = 'Списочная часть чистых Самозанятых - имеющих СВ'
report_code = 'СЗ.04'
...
sz as (
               ...
               from   all_data a
               where  coalesce(a.type_payer,'X')='SZ'
         )
        ,
non_sz as (
               select unique sicid, rfpm_id, risk_date
               from   all_data a
               where  coalesce(a.type_payer,'X')!='SZ' --or type_payer is null
         )
...
from (
      select sicid, rfpm_id, risk_date from sz
      minus
      select sicid, rfpm_id, risk_date from non_sz
     )b, sz
where b.sicid=sz.sicid and b.rfpm_id=sz.rfpm_id and b.risk_date=sz.risk_date
group by sz.sicid, sz.rfbn_id, sz.iin, sz.rfpm_id, sz.risk_date, sz.sum_avg, sz.kzd, sz.mrzp, sz.count_donation, sz.sum_all, sz.date_approve
"""
