...
report_name = '3163 - Количество получателей и сумма выплаты по регионам'
report_code = '3163'
...
SELECT
  rfbn,
  br.name,
  COUNT(DISTINCT CASE WHEN rfpm = '07030101' THEN PNCD_ID END) CNT01,
  SUM(CASE WHEN rfpm = '07030101' THEN sm END) SUM01,
  ROUND(SUM(CASE WHEN rfpm = '07030101' THEN sm END)/COUNT(DISTINCT CASE WHEN rfpm = '07030101' THEN PNCD_ID END)/Months_Between(trunc(TO_DATE(:dt_to,'YYYY-MM-DD'), 'MM'), trunc(TO_DATE(:dt_from,'YYYY-MM-DD'), 'MM')) + 1) avg01,
  COUNT(DISTINCT CASE WHEN rfpm = '07030102' THEN PNCD_ID END) CNT02,
  SUM(CASE WHEN rfpm = '07030102' THEN sm END) SUsM02,
  ROUND(SUM(CASE WHEN rfpm = '07030102' THEN sm END)/COUNT(DISTINCT CASE WHEN rfpm = '07030102' THEN PNCD_ID END)/Months_Between(trunc(TO_DATE(:dt_to,'YYYY-MM-DD'), 'MM'), trunc(TO_DATE(:dt_from,'YYYY-MM-DD'), 'MM')) + 1) avg02,
  COUNT(DISTINCT CASE WHEN rfpm = '07030103' THEN PNCD_ID END) CNT03,
  SUM(CASE WHEN rfpm = '07030103' THEN sm END) SUM03,
  ROUND(SUM(CASE WHEN rfpm = '07030103' THEN sm END)/COUNT(DISTINCT CASE WHEN rfpm = '07030103' THEN PNCD_ID END)/Months_Between(trunc(TO_DATE(:dt_to,'YYYY-MM-DD'), 'MM'), trunc(TO_DATE(:dt_from,'YYYY-MM-DD'), 'MM')) + 1) avg03,
  COUNT(DISTINCT CASE WHEN rfpm = '07030104' THEN PNCD_ID END) CNT04,
  SUM(CASE WHEN rfpm = '07030104' THEN sm END) SUM04,
  ROUND(SUM(CASE WHEN rfpm = '07030104' THEN sm END)/COUNT(DISTINCT CASE WHEN rfpm = '07030104' THEN PNCD_ID END)/Months_Between(trunc(TO_DATE(:dt_to,'YYYY-MM-DD'), 'MM'), trunc(TO_DATE(:dt_from,'YYYY-MM-DD'), 'MM')) + 1) avg04,
  COUNT(DISTINCT CASE WHEN rfpm = '07030105' THEN PNCD_ID END) CNT05,
  SUM(CASE WHEN rfpm = '07030105' THEN sm END) SUM05,
  ROUND(SUM(CASE WHEN rfpm = '07030105' THEN sm END)/COUNT(DISTINCT CASE WHEN rfpm = '07030105' THEN PNCD_ID END)/Months_Between(trunc(TO_DATE(:dt_to,'YYYY-MM-DD'), 'MM'), trunc(TO_DATE(:dt_from,'YYYY-MM-DD'), 'MM')) + 1) avg05,
  COUNT(DISTINCT CASE WHEN rfpm = '07030106' THEN PNCD_ID END) CNT06,
  SUM(CASE WHEN rfpm = '07030106' THEN sm END) SUM06,
  ROUND(SUM(CASE WHEN rfpm = '07030106' THEN sm END)/COUNT(DISTINCT CASE WHEN rfpm = '07030104' THEN PNCD_ID END)/Months_Between(trunc(TO_DATE(:dt_to,'YYYY-MM-DD'), 'MM'), trunc(TO_DATE(:dt_from,'YYYY-MM-DD'), 'MM')) + 1) avg06,
...
SELECT
 ...
 WHERE D.PNCP_DATE >= TO_DATE(:dt_from,'YYYY-MM-DD')
...
def format_worksheet(worksheet, common_format):
    ...
    worksheet.merge_range(2, 1, 2, 18, 'Количество месяцев назначения', common_format)

    worksheet.merge_range(3, 1, 3, 3, '1 месяц', common_format)
    worksheet.merge_range(3, 4, 3, 6, '2 месяца', common_format)
    worksheet.merge_range(3, 7, 3, 9, '3 месяца', common_format)
    worksheet.merge_range(3, 10, 3, 12, '4 месяца', common_format)
    worksheet.merge_range(3, 13, 3, 15, '5 месяцев', common_format)
    worksheet.merge_range(3, 16, 3, 18, '6 месяцев', common_format)
    worksheet.merge_range(2, 19, 3, 21, 'Всего', common_format)


    for start_col in [1, 4, 7, 10, 13, 16, 19]:
    ...
    with oracledb.connect(user=db_user, password=db_password, dsn=db_dsn) as connection:
        with connection.cursor() as cursor:
            ...
            for record in rows:
                ...
                worksheet[0].write(row_num, 21, record[22], money_format)
