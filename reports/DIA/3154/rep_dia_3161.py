...
report_name = '3161 - Сведения о назначенных средних размерах по областям'
report_code = '3161'
...
SELECT
    br.RFBN_ID,
    br.NAME,
    round(AVG(DISTINCT CASE WHEN rfpm_id = '07030101' THEN sfa.sum_all END), 2) AV01,
    round(AVG(DISTINCT CASE WHEN rfpm_id = '07030102' THEN sfa.sum_all END), 2) AV02,
    round(AVG(DISTINCT CASE WHEN rfpm_id = '07030103' THEN sfa.sum_all END), 2) AV03,
    round(AVG(DISTINCT CASE WHEN rfpm_id = '07030104' THEN sfa.sum_all END), 2) AV04,
    round(AVG(DISTINCT CASE WHEN rfpm_id = '07030105' THEN sfa.sum_all END), 2) AV05,
    round(AVG(DISTINCT CASE WHEN rfpm_id = '07030106' THEN sfa.sum_all END), 2) AV06,
    round(AVG(sfa.sum_all), 2) AV
  FROM SIPR_MAKET_FIRST_APPROVE_2 sfa, rfbn_branch br
   WHERE
     SUBSTR(sfa.RFBN_ID, 1, 2) || '00' = br.RFBN_ID
     AND substr(RFPM_ID,1,4)='0703'
...
def format_worksheet(worksheet, common_format):
    ...
    worksheet.merge_range(
        2, 1, 2, 6,
        'Средние размеры СВпр,\nтенге',
        common_format
    )
    worksheet.merge_range(2, 7, 3, 7, 'По области,\nтенге', common_format)

    worksheet.write(3, 1, '1 месяц', common_format)
    worksheet.write(3, 2, '2 месяца', common_format)
    worksheet.write(3, 3, '3 месяца', common_format)
    worksheet.write(3, 4, '4 месяца', common_format)
    worksheet.write(3, 5, '5 месяцев', common_format)
    worksheet.write(3, 6, '6 месяцев', common_format)
    ...
    with oracledb.connect(user=db_user, password=db_password, dsn=db_dsn) as connection:
        with connection.cursor() as cursor:
            ...
            for record in rows:
                ...
                worksheet[0].write(row_num, 7, record[8], money_format)
