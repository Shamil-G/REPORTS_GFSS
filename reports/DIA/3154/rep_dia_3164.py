...
report_name = '3164 - Структура получателей в разрезе пола'
report_code = '3164'
...
SELECT
  rfbn,
  br.name,
  COUNT(DISTINCT CASE WHEN rfpm = '07030101' AND sex = 1 THEN PNCD_ID END) CNT01M,
  SUM(CASE WHEN rfpm = '07030101' AND sex = 1 THEN sm ELSE 0 END) SM01M,
  COUNT(DISTINCT CASE WHEN rfpm = '07030101' AND sex = 0 THEN PNCD_ID END) CNT01W,
  SUM(CASE WHEN rfpm = '07030101' AND sex = 0 THEN sm ELSE 0 END) SM01W,
  COUNT(DISTINCT CASE WHEN rfpm = '07030102' AND sex = 1 THEN PNCD_ID END) CNT02M,
  SUM(CASE WHEN rfpm = '07030102' AND sex = 1 THEN sm ELSE 0 END) SM02M,
  COUNT(DISTINCT CASE WHEN rfpm = '07030102' AND sex = 0 THEN PNCD_ID END) CNT02W,
  SUM(CASE WHEN rfpm = '07030102' AND sex = 0 THEN sm ELSE 0 END) SM02W,
  COUNT(DISTINCT CASE WHEN rfpm = '07030103' AND sex = 1 THEN PNCD_ID END) CNT03M,
  SUM(CASE WHEN rfpm = '07030103' AND sex = 1 THEN sm ELSE 0 END) SM03M,
  COUNT(DISTINCT CASE WHEN rfpm = '07030103' AND sex = 0 THEN PNCD_ID END) CNT03W,
  SUM(CASE WHEN rfpm = '07030103' AND sex = 0 THEN sm ELSE 0 END) SM03W,
  COUNT(DISTINCT CASE WHEN rfpm = '07030104' AND sex = 1 THEN PNCD_ID END) CNT04M,
  SUM(CASE WHEN rfpm = '07030104' AND sex = 1 THEN sm ELSE 0 END) SM04M,
  COUNT(DISTINCT CASE WHEN rfpm = '07030104' AND sex = 0 THEN PNCD_ID END) CNT04W,
  SUM(CASE WHEN rfpm = '07030104' AND sex = 0 THEN sm ELSE 0 END) SM04W,
  COUNT(DISTINCT CASE WHEN rfpm = '07030105' AND sex = 1 THEN PNCD_ID END) CNT05M,
  SUM(CASE WHEN rfpm = '07030105' AND sex = 1 THEN sm ELSE 0 END) SM05M,
  COUNT(DISTINCT CASE WHEN rfpm = '07030105' AND sex = 0 THEN PNCD_ID END) CNT05W,
  SUM(CASE WHEN rfpm = '07030105' AND sex = 0 THEN sm ELSE 0 END) SM05W,
  COUNT(DISTINCT CASE WHEN rfpm = '07030106' AND sex = 1 THEN PNCD_ID END) CNT06M,
  SUM(CASE WHEN rfpm = '07030106' AND sex = 1 THEN sm ELSE 0 END) SM06M,
  COUNT(DISTINCT CASE WHEN rfpm = '07030106' AND sex = 0 THEN PNCD_ID END) CNT06W,
  SUM(CASE WHEN rfpm = '07030106' AND sex = 0 THEN sm ELSE 0 END) SM06W,
...
def format_worksheet(worksheet, common_format):
    ...
    worksheet.set_column(1, 28, 15)

    worksheet.merge_range(2, 0, 4, 0, 'Наименование\nрегиона', common_format)
    worksheet.merge_range(2, 1, 2, 24, 'Количество месяцев назначения', common_format)

    worksheet.merge_range(3, 1, 3, 4, '1 месяц', common_format)
    worksheet.merge_range(3, 5, 3, 8, '2 месяца', common_format)
    worksheet.merge_range(3, 9, 3, 12, '3 месяца', common_format)
    worksheet.merge_range(3, 13, 3, 16, '4 месяца', common_format)
    worksheet.merge_range(3, 17, 3, 20, '5 месяцев', common_format)
    worksheet.merge_range(3, 21, 3, 24, '6 месяцев', common_format)
    worksheet.merge_range(2, 25, 3, 28, 'Всего', common_format)


    for start_col in [1, 5, 9, 13, 17, 21, 25]:
    ...
    with oracledb.connect(user=db_user, password=db_password, dsn=db_dsn) as connection:
        with connection.cursor() as cursor:
            ...
            for record in rows:
                ...
                worksheet[0].write(row_num, 21, record[22], digital_format)
                worksheet[0].write(row_num, 22, record[23], money_format)
                worksheet[0].write(row_num, 23, record[24], digital_format)
                worksheet[0].write(row_num, 24, record[25], money_format)

                worksheet[0].write(row_num, 25, record[26], digital_format)
                worksheet[0].write(row_num, 26, record[27], money_format)
                worksheet[0].write(row_num, 27, record[28], digital_format)
                worksheet[0].write(row_num, 28, record[29], money_format)
            ...
            for col in range(1, 29):
                ...
                fmt = total_money_format if col % 2 == 0 else total_digital_format
