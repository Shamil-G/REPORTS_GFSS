...
report_name = '3145 - Градация размеров получателей'
report_code = '3145'
...
SELECT
  ...
  COUNT(CASE WHEN rfpm = '07010101' THEN PNCD_ID END) CNT01,
  COUNT(CASE WHEN rfpm = '07010102' THEN PNCD_ID END) CNT02,
  COUNT(CASE WHEN rfpm = '07010103' THEN PNCD_ID END) CNT03,
  COUNT(CASE WHEN rfpm = '07010104' THEN PNCD_ID END) CNT04,
  COUNT(PNCD_ID) CNT
FROM(
SELECT
  ...
  d.source_id,
  ph.sum_pay,
  TRUNC(sum_pay / 1000) * 1000 sum_from,
  (TRUNC(sum_pay / 1000) + 1) * 1000 sum_to
  FROM PNPD_DOCUMENT D, PAYMENT_HISTORY PH
 WHERE D.PNCD_ID = PH.PNCD_ID(+)
   AND d.knp <> '010'
   AND PH.ACT_MONTH BETWEEN TO_DATE(:dt_from,'YYYY-MM-DD') AND TO_DATE(:dt_to,'YYYY-MM-DD')
