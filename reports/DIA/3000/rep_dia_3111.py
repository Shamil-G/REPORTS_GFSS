...
report_name = '3111 - Женщины-участники СОСС 50+, СО по БИН и ИИН'
report_code = '3111'

stmt_report = """
SELECT T.AGE,
       COUNT(DISTINCT T.SICID) cnt, 
       round(SUM(T.SUM_PAY)/1000, 2) sm,
       round(SUM(T.sum_doh) / 1000, 2) sd,
       COUNT(DISTINCT CASE WHEN t.typ = 0 THEN T.SICID ELSE NULL END) cnt_bin,
       round(SUM(CASE WHEN t.typ = 0 THEN T.SUM_PAY END)/1000, 2) sm_bin,
       COUNT(DISTINCT CASE WHEN t.typ = 2 THEN T.SICID ELSE NULL END) cnt_ip,
       round(SUM(CASE WHEN t.typ = 2 THEN T.SUM_PAY END)/1000, 2) sm_ip,
       COUNT(DISTINCT CASE WHEN t.typ = 1 THEN T.SICID ELSE NULL END) cnt_iin,
       round(SUM(CASE WHEN t.typ = 1 THEN T.SUM_PAY END)/1000, 2) sm_iin
  FROM (SELECT P.SICID,
               TRUNC(MONTHS_BETWEEN(TO_DATE(:dt_to,'YYYY-MM-DD'), P.BIRTHDATE) / 12) AGE,
               sim.sum_pay,
               CASE WHEN sim.period < '01.01.2018' THEN 20 * SIM.SUM_PAY ELSE 28.57 * SIM.SUM_PAY END sum_doh,
               CASE WHEN SUBSTR(P_RNN, 5, 1) IN ('0', '1', '2', '3') THEN CASE WHEN p.iin = sim.p_rnn THEN 2 ELSE 1 END ELSE 0 END typ
        FROM SI_MEMBER_2 SIM, PERSON P
        WHERE P.SICID = SIM.SICID
        AND P.SEX = 0
        AND SIM.KNP = '012'
...
def format_worksheet(worksheet, common_format):
    ...
    worksheet.merge_range(2, 0, 4, 0, 'Возраст', common_format)
    worksheet.merge_range(2, 1, 2, 3, 'Женщины от 50+', common_format)
    worksheet.merge_range(3, 1, 3, 2, 'СО', common_format)
    worksheet.merge_range(3, 3, 4, 3, 'Исчисленный доход (тыс. тенге)', common_format)

    worksheet.merge_range(2, 4, 2, 9, 'Из них уплатили СО', common_format)
    worksheet.merge_range(3, 4, 3, 5, 'по БИН (наемные работники)', common_format)
    worksheet.merge_range(3, 6, 3, 7, 'по ИИН (самозанятые), за себя', common_format)
    worksheet.merge_range(3, 8, 3, 9, 'по ИИН (самозанятые), наемные работники', common_format)

    for start_col in [1, 4, 6, 8]:
        worksheet.write(4, start_col,     'Количество женщин', common_format)
        worksheet.write(4, start_col + 1, 'Сумма СО (тыс. тенге)', common_format)
    ...
    with oracledb.connect(user=db_user, password=db_password, dsn=db_dsn) as connection:
        with connection.cursor() as cursor:
            ...
            for record in rows:
                ...
                worksheet[0].write(row_num, 2, record[2], money_format)
            ...
            for col in range(1, 10):
                ...
                fmt = total_money_format if col in (2, 3, 5, 7, 9) else total_digital_format
