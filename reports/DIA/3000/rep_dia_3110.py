...
report_name = '3110 - Численность участников СОСС, в разрезе пола и возраста'
report_code = '3110'

stmt_report = """
SELECT 
   T.AGE,
   COUNT(UNIQUE CASE WHEN T.KNP = '012' AND T.SEX = 0 THEN T.SICID ELSE NULL END) CNT12_W,
   SUM(CASE WHEN T.KNP = '012' AND T.SEX = 0 THEN T.SUM_PAY END) SM12_W,
   COUNT(UNIQUE CASE WHEN T.KNP = '017' AND T.SEX = 0 THEN T.SICID ELSE NULL END) CNT17_W,
   SUM(CASE WHEN T.KNP = '017' AND T.SEX = 0 THEN T.SUM_PAY END) SM17_W,
   COUNT(UNIQUE CASE WHEN T.KNP = '012' AND T.SEX = 1 THEN T.SICID ELSE NULL END) CNT12_M,
   SUM(CASE WHEN T.KNP = '012' AND T.SEX = 1 THEN T.SUM_PAY END) SM12_M,
   COUNT(UNIQUE CASE WHEN T.KNP = '017' AND T.SEX = 1 THEN T.SICID ELSE NULL END) CNT17_M,
   SUM(CASE WHEN T.KNP = '017' AND T.SEX = 1 THEN T.SUM_PAY END) SM17_M
FROM (SELECT P.SICID,
           P.SEX,
...
def format_worksheet(worksheet, common_format):
    ...
    worksheet.merge_range(2, 1, 2, 4, 'Женщины', common_format)
    worksheet.merge_range(3, 1, 3, 2, 'Социальные отчисления', common_format)
    worksheet.merge_range(3, 3, 3, 4, 'Пеня', common_format)

    worksheet.merge_range(2, 5, 2, 8, 'Мужчины', common_format)
    worksheet.merge_range(3, 5, 3, 6, 'Социальные отчисления', common_format)
    worksheet.merge_range(3, 7, 3, 8, 'Пеня', common_format)

    for start_col in [1, 3, 5, 7]:
        worksheet.write(4, start_col,     'Количество участников, чел', common_format)
        worksheet.write(4, start_col + 1, 'Сумма (тыс. тенге)', common_format)
    ...
    with oracledb.connect(user=db_user, password=db_password, dsn=db_dsn) as connection:
        with connection.cursor() as cursor:
            ...
            for record in rows:
                ...
                worksheet[0].write(row_num, 8, record[8], money_format)
