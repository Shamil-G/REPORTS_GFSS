...
report_name = '3124 - Сведения о размерах выплат по районам'
report_code = '3124'
...
SELECT
  reg,
  rfbn.NAME,
  ROUND(SUM(CASE WHEN rfpm = '0701' THEN sz_avg ELSE 0 END), 2) s01,
  ROUND(SUM(CASE WHEN rfpm = '0702' THEN sz_avg ELSE 0 END), 2) s02,
  ROUND(SUM(CASE WHEN rfpm = '0703' THEN sz_avg ELSE 0 END), 2) s03,
  ROUND(SUM(CASE WHEN rfpm = '0704' THEN sz_avg ELSE 0 END), 2) s04,
  ROUND(SUM(CASE WHEN rfpm = '0705' THEN sz_avg ELSE 0 END), 2) s05,
  ROUND(SUM(CASE WHEN rfpm = '0706' THEN sz_avg ELSE 0 END), 2) s06
...
(
    SELECT
        ...
        COUNT(DISTINCT pd.source_id) cnt,
        SUM(ph.sum_pay) sa
    ...
    (
        (ph.rfbn_id, SUBSTR(ph.rfpm_id, 1, 4)),
        (SUBSTR(ph.rfpm_id, 1, 4))
    )
) t,
rfbn_branch rfbn
...
def format_worksheet(worksheet, common_format):
    ...
    worksheet.merge_range(2, 2, 2, 7, 'Средний размер', common_format)


    worksheet.write(3, 2, 'Социальная выплата на случай потери кормильца', common_format)
    worksheet.write(3, 3, 'Социальная выплата на случай утраты трудоспособности', common_format)
    worksheet.write(3, 4, 'Социальная выплата на случай потери работы', common_format)
    worksheet.write(3, 5, 'Социальная выплата на случай потери дохода в связи с беременностью и родами', common_format)
    worksheet.write(3, 6, 'Социальная выплата на случай потери дохода в связи с уходом за ребенком до года', common_format)
    worksheet.write(3, 7, 'Социальная выплата участникам системы обязательного социального страхования на период чрезвычайного положения', common_format)
        ...
        with connection.cursor() as cursor:
            ...
            for col in range(1, 8):
                ...
                worksheet[0].write_formula(
                    row_num,
                    col,
                    f'=AVERAGE({col_letter}{first_row}:{col_letter}{row_num})',
                    total_money_format
                )
