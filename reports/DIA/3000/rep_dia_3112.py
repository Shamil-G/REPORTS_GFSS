...
report_name = '3112 - Женщины-участники СОСС 50+, СО от нескольких работодателей'
report_code = '3112'

stmt_report = """
SELECT
    t.age,
    COUNT(UNIQUE T.SICID) cnt,
    COUNT(UNIQUE CASE WHEN t.rb = 1 THEN T.SICID ELSE NULL END) cnt_01,
    COUNT(UNIQUE CASE WHEN t.rb = 2 THEN T.SICID ELSE NULL END) cnt_02,
    COUNT(UNIQUE CASE WHEN t.rb = 3 THEN T.SICID ELSE NULL END) cnt_03,
    COUNT(UNIQUE CASE WHEN t.rb = 4 THEN T.SICID ELSE NULL END) cnt_04,
    COUNT(UNIQUE CASE WHEN t.rb = 5 THEN T.SICID ELSE NULL END) cnt_05
FROM (SELECT P.SICID,
            TRUNC(MONTHS_BETWEEN(TO_DATE(:dt_to,'YYYY-MM-DD'), P.BIRTHDATE) / 12) AGE,
            least(COUNT(DISTINCT sim.p_rnn), 5) rb
...
def format_worksheet(worksheet, common_format):
    ...
    worksheet.merge_range(2, 0, 3, 0, 'Возраст', common_format)
    worksheet.merge_range(2, 1, 3, 1, 'Количество женщин', common_format)


    worksheet.merge_range(2, 2, 2, 6, 'количество женщин, за которых поступили СО от нескольких работодателей', common_format)

    worksheet.write(3, 2,  '1 работодатель', common_format)
    worksheet.write(3, 3, '2 работодателя',        common_format)
    worksheet.write(3, 4, '3 работодателя', common_format)
    worksheet.write(3, 5, '4 работодателя', common_format)
    worksheet.write(3, 6, '5 и более работодателей', common_format)
