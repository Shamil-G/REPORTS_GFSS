...
report_name = '3191 - Утрата трудоспособности'
report_code = '3191'
...
SELECT
  rfbn,
  br.name,
  COUNT(DISTINCT CASE WHEN rfpm = '07020101' AND sex = 1 THEN PNCD_ID END) CNT01M,
  SUM(CASE WHEN rfpm = '07020101' AND sex = 1 THEN sm ELSE 0 END) SM01M,
  COUNT(DISTINCT CASE WHEN rfpm = '07020101' AND sex = 0 THEN PNCD_ID END) CNT01W,
  SUM(CASE WHEN rfpm = '07020101' AND sex = 0 THEN sm ELSE 0 END) SM01W,
  COUNT(DISTINCT CASE WHEN rfpm = '07020102' AND sex = 1 THEN PNCD_ID END) CNT02M,
  SUM(CASE WHEN rfpm = '07020102' AND sex = 1 THEN sm ELSE 0 END) SM02M,
  COUNT(DISTINCT CASE WHEN rfpm = '07020102' AND sex = 0 THEN PNCD_ID END) CNT02W,
  SUM(CASE WHEN rfpm = '07020102' AND sex = 0 THEN sm ELSE 0 END) SM02W,
  COUNT(DISTINCT CASE WHEN rfpm = '07020103' AND sex = 1 THEN PNCD_ID END) CNT03M,
  SUM(CASE WHEN rfpm = '07020103' AND sex = 1 THEN sm ELSE 0 END) SM03M,
  COUNT(DISTINCT CASE WHEN rfpm = '07020103' AND sex = 0 THEN PNCD_ID END) CNT03W,
  SUM(CASE WHEN rfpm = '07020103' AND sex = 0 THEN sm ELSE 0 END) SM03W,
...
def format_worksheet(worksheet, common_format):
    ...
    worksheet.merge_range(2, 2, 2, 13, 'Степень утраты трудоспособности, %', common_format)

    worksheet.merge_range(3, 2, 3, 5, '80-100', common_format)
    worksheet.merge_range(3, 6, 3, 9, '60-80', common_format)
    worksheet.merge_range(3, 10, 3, 13, '30-60', common_format)
    worksheet.merge_range(2, 14, 3, 17, 'Всего', common_format)

    for start_col in [2, 6, 10, 14]:
